...
s3_base_url = os.getenv("WASABI_ENDPOINT_URL") # New url @Pushkar
...
if __name__ == "__main__":
    images_path = '/root/comfyui-frameworks/input/input'
    save_path = '/root/comfyui-frameworks/input_url.json'
    d = {}
    for id in os.listdir(images_path):
        image_path = os.path.join(images_path, id)
        prod_img = upload_wasabi_rest(image_to_base64(image_path), f"{uuid.uuid4()}.jpg")
        print(prod_img)
        if prod_img != 500:
            d[id] = prod_img
        break
